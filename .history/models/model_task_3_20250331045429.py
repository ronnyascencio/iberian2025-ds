...
def train_anomaly_detector(df_filtered):
    ...
    try:
        ...
        for clase in np.unique(y_train):
            ...
            optimal_params = grid_model_bin.best_params_
...
def fit_binary_model(X_train_bin, X_test_bin, y_train_bin, grid_model_bin):
    ...
    # CORREGIDO: Usar indexación directa para y_train_bin
    y_sample_bin = y_train_bin[sample_idx_bin]  # Cambiado de .iloc a indexación directa
    
    ratio = (y_sample_bin == 0).sum() / (y_sample_bin == 1).sum()
    sample_weight_bin = np.where(y_sample_bin == 1, ratio, 1)
    
    # CORREGIDO: Asegurar que sample_weight_bin tenga la longitud correcta
    # Eliminar esta línea si causa problemas:
    # sample_weight_bin = sample_weight_bin[:len(X_sample_bin)]
    
    # Fit the grid search model
    grid_model_bin.fit(X_sample_bin, y_sample_bin, sample_weight=sample_weight_bin)
    
    # Extract the best model with optimal hyperparameters
    model_bin = grid_model_bin.best_estimator_
    
    # CORREGIDO: Crear pesos específicos para los datos completos de entrenamiento
