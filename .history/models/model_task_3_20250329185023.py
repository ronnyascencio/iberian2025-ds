...
def data_filter(StatCode, ItCode, end_date, start_date):
    ...
    merged_df["Measurement date"]
     start_date = pd.to_datetime(start_date)
    

    merged_df["Measurement date"] = pd.to_datetime(merged_df["Measurement date"])

    return merged_df[
    (merged_df["Station code"] == StatCode) &
    (merged_df["Item code"] == ItCode) &
    (merged_df["Measurement date"] >= start_date) &
    (merged_df["Measurement date"].todate_time <= end_date)
]
...
def machine_learning(df_filtered):
    ...
    X = df_filtered[["Average value", "hour", "dayofweek"]].dropna()
    X.describe()
...
for each in input_list:
    ...
    df_filtered = data_filter(station_code, pollutant_code, end_date, start_date)
