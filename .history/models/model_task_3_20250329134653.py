...
import pandas as pd
# prepare input/s
line = "Station code: 205 | pollutant: SO2  | Period: 2023-11-01 00:00:00 - 2023-11-30 23:00:00"
parts = line.split("|")

station_code = int(parts[0].split(":")[1].strip())
pollutant = parts[1].split(":")[1].strip()
start_date, end_date = parts[2].split("Period:")[1].strip()
...
def filter(StatCode, ItCode, periodDates):
    # filter for each station code:
    return(merged_df[merged_df["Station code"]==StatCode & merged_df[merged_df["Item code"]== ItCode]] &
           merged_df[merged_df["Measurement date"] in periodDates])
