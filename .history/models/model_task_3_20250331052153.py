...
def train_anomaly_detector(df_filtered):
    ...
    try:
        ...
        # Removed the return statement as it is outside of a function
        pass
    except ValueError as e:
        print(f"Error al dividir los datos: {e}")
        if 'y' in locals() and len(np.unique(y)) < 2:
...
def fit_binary_model(X_train_bin, y_train_bin):
...
def fit_binary_model(X_train_bin, X_test_bin, y_train_bin):
    ...
    try:
        ...
        df_features = prepare_features(merged_df)
...
def train_anomaly_detector(df_filtered):
    try:
        # ... aquí va el código de entrenamiento del modelo, por ejemplo:
        # X_train, X_test, y_train, y_test = train_test_split(...)
        # model = XGBClassifier(...); model.fit(X_train, y_train, sample_weight=sample_weight)
        # anomalies = df_features[model.predict(X) == 1]
        return model, anomalies
