...
def train_anomaly_detector(df_filtered):
    ...
    try:
        ...
        for clase in np.unique(y_train):
            ...
            model_bin = train_binary_model(X_train_bin, y_train_bin)
            explain_binary_model(model_bin, X_test_bin, class_error)

            y_proba_bin = model_bin.predict_proba(X_test_bin)[:, 1]
            y_pred_bin = (y_proba_bin > 0.95).astype(int)

            evaluate_binary_model(model_bin, X_bin, y_test_bin, y_pred_bin, class_error)
            i += 1
    ...
    def train_binary_model(X_train_bin, y_train_bin):
    ...
    def explain_binary_model(model_bin, X_test_bin, class_error):
        ...
        for name, val in zip(X_test_bin.columns, mean_shap):
        print(f"{name}: {val:.4f}")


    def evaluate_binary_model(model_bin, X_bin, y_test_bin, y_pred_bin, class_error):
